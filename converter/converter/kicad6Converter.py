...
class KiCad6Converter(Converter):
    ...
    with open(CUR_ABS_DIR / "../symbols/kicad_classes.json") as classes_file:
        ...
        for node in graph.nodes:
            ...
            if node_class[node] == "junction":
                ...
                if node_type[node] != 'junction' and node_type[node] != 'extra_junction':
                    symbol_instances[node] = {'symbol_uuid': symbol_uuid, 'reference': reference, 'unit': 1,
                                              'value': node_path[node]['value'], 'footprint': footprint}
