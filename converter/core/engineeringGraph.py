...
from converter.core.geometry import transform, transform_to_local, Point, map_points, length, shift, scale
# Project Imports
from converter.core.symbol import load_symbols, Port, Property
...
class EngGraph(nx.DiGraph):
    ...
    with open(CUR_ABS_DIR / "../symbols/properties.json") as prop_map_file:
    ...
    def add_node(self, node_id=None, name="", type="", position=None, text="", ports=None, shape=None, properties=None):
        """Protective Wrapper for Enforcing the Datamodel"""
        ...
        super(EngGraph, self).add_node(node_id, name=name, type=type, position=position, text=text,
                                       ports=[], shape=shape, properties=[])
    ...
    def add_edge(self, source, target, sourcePort=None, targetPort=None, shape=None, type="electrical"):
        """Protective Wrapper for Enforcing the Datamodel"""
    ...
    def add_port(self, node_id, position: dict = None, name: str = None, port: Port = None):
        ...
        ports.append(Port(name, transform_to_local(Point(position['x'], position['y']), node_pos)))
    ...
    def add_symbol_ports(self, node_id, match_existing: bool = True) -> None:
        ...
        if node_type in self.symbols:
            ...
            existing_port_pos = {port.position: port for port in self.nodes[node_id]['ports']}
            ...
            for symbol_port in symbol_ports:
                if symbol_port.position in point_map.keys():
                    self.replace_port(node_id, existing_port_pos[point_map[symbol_port.position]], symbol_port)
    ...
    def closestPort(self, position: Point) -> tuple:
        ...
        for node_id, node_attrs in self.nodes.items():
            for port in node_attrs['ports']:
                dist_test = length(shift(self.port_position(node_attrs['position'], port), scale(position, -1, -1)))
    ...
    def closestNode(self, pos: dict, electrical_only: bool = False, ppMetric=False) -> int:
        ...
        for node_id, node_pos in nx.get_node_attributes(self, 'position').items():
            ...
            if not electrical_only or (electrical_only and node_type[node_id] not in ["junction", "crossover", "text"]):

                if ppMetric:
                    dist = sqrt((node_pos['x'] - pos['x']) ** 2 + (node_pos['y'] - pos['y']) ** 2)

                else:
                    dist = -cv2.pointPolygonTest(
                        np.array([(node_pos['x'] - node_pos['width'] // 2, node_pos['y'] - node_pos['height'] // 2),
                                  (node_pos['x'] + node_pos['width'] // 2, node_pos['y'] - node_pos['height'] // 2),
                                  (node_pos['x'] + node_pos['width'] // 2, node_pos['y'] + node_pos['height'] // 2),
                                  (node_pos['x'] - node_pos['width'] // 2, node_pos['y'] + node_pos['height'] // 2)],
                                 dtype=np.int32),
                        (pos['x'], pos['y']),
    ...
    def edgeEndPoint(self, node_id, port_id=None):
        ...
        if port_id is None:
            return self.nodes[node_id]['position']['x'], self.nodes[node_id]['position']['y']

        else:
            port = [port for port in self.nodes[node_id]['ports'] if port.name == port_id][0]
    ...
    def edgeEndPoints(self, usePorts: bool = True, convertToInt: bool = False):
        ...
        for source, target, edge_data in self.edges(data=True):
            source_point = self.edgeEndPoint(source, edge_data.get('sourcePort', None) if usePorts else None)
            target_point = self.edgeEndPoint(target, edge_data.get('targetPort', None) if usePorts else None)
    ...
    def move_node(self, node: int, delta_x: NUMBER, delta_y: NUMBER, update_mask: bool = True):
    ...
    def resolve_text_nodes(self):
        ...
        for node_id, node_data in self.nodes.items():
            if node_data['type'] == "text" and node_data.get("text", ""):
                ...
                prop = Property(name=self.classify_text(node_data['text']),
                                value=node_data['text'],
                                position=transform_to_local(Point(node_data['position']['x'],
        ...
        for node_id, node_type in nx.get_node_attributes(self, "type").items():
            if node_type == "crossover":
                if self.degree(node_id) == 4:
                    neighbors = [(source, data.get("sourcePort", None), target, data.get("targetPort", None))
                                 for source, target, data in self.in_edges(node_id, True)] + \
                                [(target, data.get("targetPort", None), source, data.get("sourcePort", None))
                                 for source, target, data in self.out_edges(node_id, True)]
                    endpoints = [(self.edgeEndPoint(other, other_port), self.edgeEndPoint(own, own_port))
                                 for other, other_port, own, own_port in neighbors]
                    displacement = [(other[0] - own[0], other[1] - own[1])
                                    for other, own in endpoints]
                    angle = [atan2(vector[1], vector[0]) for vector in displacement]
                    angle_dist = [abs(((a - angle[0] + pi) % (2 * pi)) - pi) for a in angle]
                    zero_partner_index = angle_dist.index(max(angle_dist))
                    neighbors[1], neighbors[zero_partner_index] = neighbors[zero_partner_index], neighbors[1]

                    self.add_edge(neighbors[0][0], neighbors[1][0],
                                  sourcePort=neighbors[0][1], targetPort=neighbors[1][1])
                    self.add_edge(neighbors[2][0], neighbors[3][0],
                                  sourcePort=neighbors[2][1], targetPort=neighbors[3][1])
                    self.remove_node(node_id)

                else:
                    print(f"Can't resolve crossover {node_id} with degree {self.degree(node_id)}")
...
def geo_resolve(other: Union[int, float, Tuple[NUMBER, NUMBER]]) -> Tuple[NUMBER, NUMBER]:
    ...
    elif type(other) is tuple and len(other) == 2 and \
            (type(other[0]) is int or type(other[0]) is float) and \
            (type(other[1]) is int or type(other[1]) is float):
        other_x = other[0]
        other_y = other[1]

    # Other Types are not Supported
    else:
        print("ERROR: Geometric Manipulation only allowed by numbers and pairs of numbers")
