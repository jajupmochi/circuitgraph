...
class SegmentationPixelProcessor(Processor):
    ...
    def pre_process(self, img_raw: ndarray, img_map: ndarray) -> List[Tuple[Tensor, Union[Tensor, any]]]:
        ...
        if self.train:
            ...
            while (fg_count < self.samples_per_image/2) or (bg_count < self.samples_per_image/2):
                offset_x = randint(0, img_width - self.patch_size)
                offset_y = randint(0, img_height - self.patch_size)
                center_map = img_map[offset_y + self.patch_size//2, offset_x+self.patch_size//2]
                patch_raw = img_raw[offset_y:offset_y+self.patch_size, offset_x:offset_x+self.patch_size]

                if center_map == 0 and fg_count < self.samples_per_image/2:
                    samples += [(self.transform(patch_raw), Tensor([[[1]]]))]
                    fg_count += 1

                if center_map == 255 and bg_count < self.samples_per_image/2:
                    samples += [(self.transform(patch_raw), Tensor([[[0]]]))]
                    bg_count += 1

        else:
            samples += [(self.transform(img_raw), None)]
