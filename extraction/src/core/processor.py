...
class Processor(ABC):
    ...
    @abstractmethod
    def pre_process(self, image: ndarray, graph: EngGraph) -> List[Tuple[Tensor, Tensor, any]]:
    ...
    def inference(self, image: ndarray, graph: EngGraph) -> None:
        ...
        for tensor_in, _, data_info in data_in:
            try:
                ...
                data_out += [(pred.to(self.cpu) if type(pred) is torch.Tensor else pred, data_info)]
